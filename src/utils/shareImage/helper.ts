...
export const fontEn = (): Promise<ArrayBuffer> => {
  if (preLoadEnFontData) {
    return Promise.resolve(preLoadEnFontData);
  }
  return fetch("/fonts/EBGaramond-BoldItalic.ttf").then((res) => {
    preLoadEnFontData = res.arrayBuffer();
    return preLoadEnFontData;
  });
};

export const fontZh = (): Promise<ArrayBuffer> => {
  if (preLoadZhFontData) {
    return preLoadZhFontData;
  }
  return fetch("/fonts/SourceHanSerifSC-Bold.otf").then((res) => {
